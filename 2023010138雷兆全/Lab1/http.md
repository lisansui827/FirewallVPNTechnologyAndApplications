...
2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？
   ...
   > 答：404：资源未找到，服务器上不存在请求的页面或文件。
500：服务器内部程序出错，代码异常导致无法处理请求。
503：服务器暂时不可用，通常是过载、维护，不是代码错误。

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：GET：参数拼接在 URL 后，长度受限，可缓存、可收藏，安全性低，用于查询、获取数据。
POST：参数放在请求体中，长度无明显限制，不可缓存，相对更安全，用于提交表单、上传、登录、新增修改数据。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：HTTP 明文传输，不安全；HTTPS 加密传输，更安全，默认端口不同（80/443）。
HTTPS 基于 SSL/TLS 加密，结合对称加密、非对称加密、数字证书保证数据机密性与完整性。

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：浏览器是加密通信的一端，数据在发送前 / 接收后会被解密，所以开发者工具能看到明文。
